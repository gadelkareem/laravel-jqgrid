...
class JqGridRender implements RenderInterface
{
     ...
     * @param
     */
    public function __construct(array $optionValidators = [], array $colModelValidators = [], array $navigatorValidators = [], array $filterToolbarValidators = [], array $defaultGridOptions = [], array $defaultColModelProperties = [], array $defaultNavigatorOptions = [], array $defaultfilterToolbarOptions = [], array $functionTypeProperties = [], array $defaultFilterToolbarButtonsOptions = [])
    {
        ...
        $this->navigatorEditOptions = [];

        $this->navigatorAddOptions = [];

        $this->navigatorDeleteOptions = [];

        $this->navigatorSearchOptions = [];

        $this->navigatorViewOptions = [];
    ...
    public function setGridOption($option, $value)
    {
        foreach ($this->optionValidators as $validator) {
            $validator->validate(array_add([], $option, $value));
        }

        $property = array_add([], $option, $value);
    ...
    public function setNavigatorOptions($module, array $options)
    {
        foreach ($this->navigatorValidators as $validator) {
            $validator->validate(array_add([], $module, $options));
        }
    ...
    {
        foreach ($this->filterToolbarValidators as $validator) {
            $validator->validate(array_add([], $event, $code));
        }

        $this->filterToolbarOptions = array_merge($this->filterToolbarOptions, array_add([], $event, '###'.$code.'###'));
